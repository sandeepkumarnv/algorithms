...
public class DijkstrasShortestPath {
    ...
    private static Map<Vertex<Integer>,Integer> myFindShortestPath(Graph<Integer> g, Vertex<Integer> startVertex) {
        ...
        while(!minHeap.empty()) {
            ...
            distance.put(minNode.key,minNode.weight);
            Vertex<Integer> current = minNode.key;
            for (Edge<Integer> edge : current.getEdges()) {
                Vertex<Integer> vertex = edge.getVertex2();
                if(!minHeap.containsData(vertex)){
                    continue;
                }
                int newDistance = distance.get(current)+edge.getWeight();
                if(minHeap.getWeight(vertex) > newDistance){
                    minHeap.decrease(vertex,newDistance);
